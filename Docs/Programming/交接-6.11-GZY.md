...
为 Gameplay 系统和夜市/摊位系统创建最小可编译代码骨架。
...
6. 夜市总吸引力和总卫生值使用 `float`，因为它们属于加权/平均性质的数据。
7. 夜市2级“客流承载+20%”、3级“解锁高端摊位”、4级“口碑加成”已经提供只读规则接口，具体结算等待客流、经济或口碑系统接入。
8. 夜市停业回合通过 `closedRounds` 维护，停业时不能建设或升级摊位，夜晚结算系统应跳过该夜市。
...
4. 夜市总吸引力、总卫生值目前先按摊位基础值进行本地维护，后续需要 `DataManager` 提供完整摊位配置，并由客流、经济、检查、口碑等系统做最终修正。
5. README 中的启动场景路径 `Assets/_Project/CampusNightMarket/Scenes/S00_Bootstrap.unity` 当前不存在，仓库目前只有 `Assets/Scenes/SampleScene.unity`。
